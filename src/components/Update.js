...
function Update() {
  ...
  useEffect(() => {
    dateFound.current = Object.values(state.dates).find((item) => item.id === id && item.details === details && item.atention === atention);
        ...
        <div className='flex items-center'>
          ...
          <select 
            ...
            {optionProfesional.map(item => {
              ...
              if(form.speciality === 'Podología' && (item === 'Ana Cuadros' || item === 'Ponchi Merengueti') ){
                return null;
              }
      ...
      <div className={styles.container2}>
        ...
        <div className={styles.container3}>
            ...
            <div className={styles.btncontainer1}>
              <button onClick={handleUpdate} className='button button--gray'>ACTUALIZAR</button>
              <button onClick={handleCancel} className='button button--red'>CANCELAR</button>
            </div>
    </form>
  )
}
